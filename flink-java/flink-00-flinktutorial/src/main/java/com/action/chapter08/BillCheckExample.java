...
/**
 * 实时对账示例：使用 CoProcessFunction 和 ValueState 在两条流之间做账单匹配。
 * 核心逻辑：
 * 1. appStream 和 thirdpartStream 分别代表 App 与第三方渠道日志，按订单号 keyBy。
 * 2. 某条流事件先到，则写入状态并注册事件时间定时器，等待另一条流到齐。
 * 3. onTimer 中检查状态，仍未匹配的订单输出报警信息。
 */
